...
class followersViewController: UIViewController,UITableViewDataSource,UITableViewDelegate{
    ...
    func getUsersFromDatabase() {
        ...
        ref.child("users/profile/").child(uid).child("followers").queryOrderedByKey().observeSingleEvent(of: .value, with: { snapshot in
            
            if let following = snapshot.value as? [String : AnyObject] {
                
                for (_, value) in following {
                    ref.child("users/profile/").child("\(value)").observeSingleEvent(of: .value, with: { snapshot in
...
extension followersViewController: followUserButtonFollowers{
